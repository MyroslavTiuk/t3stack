...
const CalendarView = () => {
  ...
  useEffect(() => {
    ...
    const stocksGroup = data.equityTransaction.reduce((groups, stock) => {
    ...
    const optionsGroups = data.optionTransaction.reduce((groups, stock) => {
      const date = stock.transactionDate.toDateString().split("T")[0];
      // eslint-disable-next-line @typescript-eslint/ban-ts-comment
      // @ts-ignore
      if (!groups[date]) {
        ...
        return {
          ...
          value:
            option.options
              ...
              .reduce((acc: any, price: any) => (acc ?? 0) + (price ?? 0)) ?? 0,
