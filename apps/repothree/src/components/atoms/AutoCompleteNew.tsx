...
const AutoCompleteNew: React.FC<Props> = ({ options, onChange, value }) => {
  ...
  return (
    <Combobox onChange={onChange} value={value}>
      <div className="relative w-[376px] cursor-default  rounded-md">
        <Combobox.Input
          className="inline-flex w-full items-center justify-start gap-2.5 rounded-lg border border-gray-300 bg-gray-50 px-4 py-2 pl-10 focus:border-none focus:ring-1 focus:ring-teal-600"
      ...
      <Transition
        ...
        leaveTo="opacity-0"
      >
        <Combobox.Options className="absolute z-20 w-full rounded-lg bg-white py-1 shadow-lg ring-1 ring-teal-600 ring-opacity-5 focus:outline-none sm:text-sm">
          <Options
