...
import { OptionAction, Position, TransactionSource } from "opcalc-database";
import {
  tradePageSize,
...
import { getOptionExpirationDate } from "~/server/tradier";
...
export const transactionRouter = createTRPCRouter({
  ...
  getOptionExpiration: protectedProcedure
    .input(z.object({ symbol: z.string() }))
    .query(async ({ ctx, input }) => {
      try {
        const futureDates = await getOptionExpirationDate(input.symbol);
        console.time("finising query");
        const previousRawDates = (await ctx.prisma.$queryRaw`
            SELECT DISTINCT expiration
            FROM historical_data
            WHERE "underlying_symbol" = ${input.symbol}
            AND expiration > CURRENT_DATE - interval '60 days'
            ;`) as {
          expiration: string;
        }[];
        console.timeEnd("finising query");
        const previousDates: ExpirationDate[] = previousRawDates.map((d) => ({
          date: new Date(d.expiration),
        }));
        let expirationDates = [...previousDates, ...futureDates];
        // @ts-ignore
        expirationDates.sort((lhs, rhs) => lhs.date - rhs.date);
        return expirationDates;
      } catch (e) {
        throw new TRPCError({
          code: "NOT_FOUND",
          message: `Can not retrieve previous expiration dates from DB, error: ${e}`,
        });
      }
    }),
});
